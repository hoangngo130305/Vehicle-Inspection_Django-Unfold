...
class OrderSerializer(serializers.ModelSerializer):
    ...
    # ✅ FIX: Customer field không bắt buộc (cho phép Customer tự tạo order)
...
class VehicleReceiptInitializeSerializer(serializers.Serializer):
    ...
    Request: {} (empty)
    Response: { receipt_id, status: 'draft' }
    """
    pass  # Không cần fields, chỉ cần order_id từ URL
...
class VehicleReceiptConditionCheckSerializer(serializers.Serializer):
    """
    ✅ API #3: Condition Check - Lưu 10 checkbox + 10 ảnh checklist
    POST /api/orders/{id}/vehicle-receipt/condition-check/
    PUT  /api/orders/{id}/vehicle-receipt/condition-check/
    
    Request: { 10 checkbox + 10 ảnh checklist }
    
    UPDATED: 10/03/2026 - Thêm 2 checkbox + 2 ảnh giấy tờ & tem
    
    NOTE: Các fields mô tả text (exterior_front, windows_condition, etc.) 
    tồn tại trong database nhưng KHÔNG được sử dụng trong API này vì:
    - Giao diện chỉ có checkbox + ảnh
    - Không có input text để nhập mô tả
    - Chúng sẽ luôn rỗng ('')
    """
    # 8 checkbox chính
